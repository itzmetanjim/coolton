...
`deploy.sh` is the Slack CLI deploy hook: it updates `.env` with freshly-issued tokens and
restarts `coolton.service`.
...
run `ruff check` on changed files and the full `pytest` suite before every change. it's the
CI-shaped habit this repo is built around.
...
`timeout`. `run_linux_command` clamps to 10 to 1800s by default, `0` disables it entirely.
...
- **never** run untrusted or model-supplied code on the host. it belongs in the E2B sandbox, the
