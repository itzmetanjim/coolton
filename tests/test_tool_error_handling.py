...
def test_sandbox_file_tools_report_connect_errors():
    with patch("agent.tools.sandbox_files.Sandbox.connect", side_effect=RuntimeError("sandbox expired")), \
         patch("agent.tools.sandbox_files.get_thread_sandbox_id", return_value="sbx-1"):
        sandbox, error = _get_sandbox("C123", "1.2")
    assert sandbox is None
    assert error == "Error connecting to sandbox: sandbox expired"
